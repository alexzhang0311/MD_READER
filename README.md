...
一个基于 Flask 的 Web 端知识库阅读器。支持 Markdown / YAML 文件目录浏览、渲染展示、代码高亮和用户认证。
...
| 功能 | 说明 |
|------|------|
| 📂 文件树浏览 | 支持单目录或多根目录（根分组）展示 `.md` / `.markdown` / `.yaml` / `.yml` 文件，支持多级目录折叠 |
| 📖 文档渲染 | Markdown 使用 GFM 扩展语法渲染；YAML 以语法高亮代码块方式展示 |
| 🎨 代码高亮 | 基于 Pygments 的语法高亮（Monokai 主题），支持数十种语言 |
| ⬇️ 文件下载 | 支持直接下载当前查看的 Markdown / YAML 原文件 |
| 🔍 文件搜索 | 侧边栏实时搜索过滤文件名 |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/files` | 获取支持文件树（Markdown / YAML，JSON，支持多根分组） |
| GET | `/api/read?path=<相对路径>` | 读取并渲染指定文件（多根模式下例如 `根名/文件.md` 或 `根名/配置.yaml`） |
| GET | `/api/download?path=<相对路径>` | 下载 Markdown / YAML 原文件 |
